echo  "@font-face{font-family:"revicons";fallback:fallback;src:url("./revicons.woff") format('woff'),url("./revicons.ttf") format('ttf'),url("./revicons.eot") format('ttf')}
...
.react-multiple-carousel__arrow{position:absolute;outline:0;transition:all .5s;border-radius:35px;z-index:0;border:0;min-width:43px;min-height:43px;opacity:1;cursor:pointer }
.react-multiple-carousel__arrow:hover{background:rgba(0,0,0,0.5)}
.react-multiple-carousel__arrow::before{font-size:20px;color:white;display:block;font-family:revicons;text-align:center;z-index:0;position:relative}
